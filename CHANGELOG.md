...
- Diagnosed why the daily "link favorite styles to samples" automation was failing about half the time — it was trying to link the sample by matching on a name that Airtable computes automatically and can never match. Rebuilt it to match on the actual style instead, and to link every matching sample it finds, not just one. Code is ready; still needs to be pasted into the live automation.
