...
- Clicking a Pick Up appointment now shows a table of that client's orders (order #, payment, delivery method, readiness, total, order status) right in the appointment detail view — first step of a larger change that will let reception mark individual orders as picked up or switch one from Pick Up to Ship straight from Appointments. Read-only for now; still needs a few answers from Cobalt before the marking/switching part goes live.
- Clicking one of those orders opens its full detail page (readiness checklist, fulfillment info, order items, adjustments, financials, sync history) — the same order detail page already used in Fulfillment, so it looks and works the same in both places, scoped down slightly for Appointments (a few fields that only apply to Fulfillment's own workflow are hidden or read-only here).
- Fixed the client's Orders table not showing up (or showing empty) on Pick Up appointments — it was reading the wrong appointment-type field to decide whether to show the table at all, and separately wasn't loading the client's linked orders.
