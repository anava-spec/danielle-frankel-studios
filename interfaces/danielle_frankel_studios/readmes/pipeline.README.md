...
- 2026-08-21 (Order Ready section, Issues 3 & 4 of the "Pipeline — Order Ready, Alterations & Fulfillment" batch): row order is now Items Sold / (Order Ready, Alterations, Client Notified — one `FieldRow`, equal width) / Customization Notes / an inline Orders table.
  ...
  - **Issue 4** — `Client Notified` in this section is now always read-only (regardless of the section's editable/read-only mode) and rendered as the same `ProgressBar`, driven by a new rollup field `client_notified_percentage` (`fldKSGxe3uMdjHWz5`, **Sandbox only as of 2026-08-21** — Axel replicates the same config in Production before it goes live there): rolls up `Orders.client_notified` across `shopify_order` with formula `IF(COUNT(values), SUM(values)/COUNT(values))`, i.e. the fraction of the bride's orders where the client has been notified. The old editable checkbox on this same underlying field (`client_notified_fulfillment`, `fldxumxeRnrDQ3CIk`) is untouched and still live in the **In Fulfillment** section further down — that's a separate, not-yet-tackled issue (per-order Fulfillment Method/Client Notified/Tracking/3PL at bride-level), don't assume it was also fixed.
