...
1. **Qualification floor (always-on, not a filter toggle):** a client only ever appears on this list if `Item Category` (`fldE4AX5gz8pwah4j`) contains at least one of `gown`, `custom`, `top`, `skirt`, `pant`, `dress` (case-insensitive substring match). Clients whose `Item Category` is shoes/veil-only, or empty, never qualify — this is a hard floor applied inside `filteredRecords`, before any of the interactive filters below run. Approved deviation from the original AC: "direct-to-consumer" isn't a separate flag on `DF Clients` yet, so for now this Item Category floor is the only DTC-qualification signal. **Corrected from an earlier version of this floor that checked `Items Sold` instead** — `Items Sold` doesn't carry clean per-category text (see the lookup-parsing note below), so it was the wrong field to gate on; `Item Category` is a lookup chain that terminates in a real Airtable `singleSelect` (choices: `GOWN, TOP, JEWELRY, ALTERATIONS, DRESS, SHOES, CUSTOM, PANT, SKIRT, VEIL, LOUNGE, JUMPSUIT, JACKET` — note singular `PANT`, not "pants").
