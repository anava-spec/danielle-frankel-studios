...
- The detail popup's title bar shows the client name and a "Move to X" action; the body shows the full status flow as a horizontal stepper, Wedding Date/Due Date/Gown/Dress Year, Items Sold as plain text, and Comments.
...
8. On the list, clicking the Status pill opens a small dropdown (`StatusPillDropdown`) listing every status as a colored pill option — the user picks the value explicitly. In the detail popup, status only ever advances one step forward via the title bar's "Move to X" button (see Rules) — the dropdown on the list is a faster path for correcting a status directly, the popup enforces the sequential flow.
...
- **The detail page is a popup (`ClientDetailModal`), not a full page (corrected 2026-07-30).** Fade+scale in/out per BRANDING.md §12, 720px-class width, closes via the X in the title bar, backdrop click, or Escape — no "Go back" button (there's nowhere to navigate back to; a modal doesn't need page-style back nav).
- **Title bar holds both the client name and the "Move to X" action** (renamed from "Advance to X"), not the body. Only ever offers the *next* choice in schema order — there is no way to jump to an arbitrary status from the modal (the list's `StatusPillDropdown` still allows picking any value directly, for corrections). Clicking it opens `AdvanceConfirmModal` (`z-[60]`, stacked above the detail modal's `z-50`), a deliberately minimal confirmation: one short line of text and a single "Continue" button — **no X, no Cancel, no backdrop-dismiss**, by explicit request. This is an intentional deviation from every other modal in this project (which all support backdrop/Escape/X dismissal) — don't "fix" it to match them.
- **`StatusStepper`** renders the full choice list as horizontal dots-and-connecting-line, same visual language as `pipeline.tsx`'s "Stage in pipeline" component on the Full Client Profile — colored from each choice's real Airtable color, resolved to actual hex via `resolveStatusHex`/`AIRTABLE_HEX` (a separate hex map from `AIRTABLE_COLOR_MAP`, needed because the connecting line **progressively gradients** from one step's color into the next's, not just a solid on/off line — flat Tailwind classes can't do that). Light/Bright color tiers escalate to their family's Dark1 for contrast, same convention as `pipeline.tsx`'s own stage stepper.
- **Section headers and the stepper's status labels both use `text-sm`**, matching `pipeline.tsx`'s own section-header size (`text-xs` elsewhere in this file is for field *labels*, not section headers) — deliberately kept consistent between the two per request.
- **Detail modal layout:** BRANDING.md-style `DetailSection`/`FieldRow`/`DetailRow` (same shape as `did_not_convert.tsx`), but `FieldRow` now takes a `cols` prop instead of being fixed at 3 — the "Details" section is Wedding Date / Due Date / Gown / Dress Year (4 across, Due Date has a hover tooltip explaining it's 3 months before the wedding date), then Items Sold alone on its own full-width row, **shown as plain comma-joined text, not pills** (per request — pills are still used on the main list's Items Sold/Gown columns, just not here). Comments gets its own full-width section below, editable via `CommentsCell`, which now auto-grows its height to fit long text instead of scrolling inside a fixed box.
