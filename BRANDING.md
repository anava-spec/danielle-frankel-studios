...
- **Chips with no live Airtable choice color to read** (e.g. a linked-record field like a category, which has no per-choice color at all): use a hardcoded rainbow — an evenly-spread hue per option, computed as `hue = 360 * index / totalOptionCount` over the currently-active option list (sorted stably, e.g. alphabetically, so a given option's hue doesn't shift as unrelated records load) — same solid-background/white-text rendering as above.
