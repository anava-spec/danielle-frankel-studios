...
- **Chips with no live Airtable choice color to read** (e.g. a linked-record field like a category, which has no per-choice color at all): use a hardcoded rainbow — but cycle through Airtable's own real choice-color hexes (the Bright tier of the `getChoiceColorHex` map: `#2D7FF9` blue, `#18BFFF` cyan, `#00D2C4` teal, `#20C933` green, `#F6BE00` yellow, `#FF9D00` orange, `#F94343` red, `#FF08C2` pink, `#8B46FF` purple, `#6B7280` gray — in that fixed order, `index % 10`), not a synthetic evenly-spread HSL hue (`hue = 360 * index / totalOptionCount`, deprecated 2026-09-01 — Axel supplied Airtable's actual palette swatch grid as the reference). Order over the currently-active option list, sorted stably (e.g. alphabetically) so a given option's hue doesn't shift as unrelated records load. Same solid-background/dark-text (`#1D1F25`) rendering as above — this palette's Bright tier reads fine with dark text, no white-text exception needed. Implemented as `getRainbowHex`/`RAINBOW_PALETTE` (`refund_requests.tsx`), `getRefundRainbowHex`/`REFUND_RAINBOW_PALETTE` (`draft_orders.tsx`), `getSoldRainbowHex`/`SOLD_RAINBOW_PALETTE` (`sold_orders.tsx`) — same 10 hexes, copy-pasted per file (no shared module between Interface Extension files).
