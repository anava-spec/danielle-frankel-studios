...
- **Rendering (revised 2026-09-01, per Axel): background = the solid `color` (100% opacity, no alpha suffix), text = dark/near-black (`#1D1F25`), not white** — corrected same-day after an initial white-text pass misread the reference screenshot. This supersedes the previous `color + '20'` alpha-background / solid-text treatment — that version is deprecated, migrate it on sight. `999px` radius, `4px 10px` padding for `md`, `2px 8px` for `sm`. Font size matches whatever plain text sits beside it in that context (e.g. `text-sm` next to `text-sm` table cells) — a chip reading visibly smaller than its neighboring plain text is a bug. Applied so far in `refund_requests.tsx`, `draft_orders.tsx`, `sold_orders.tsx` — migrate other interfaces (`pipeline.tsx`, `customization_requests.tsx`, etc.) to this treatment the next time they're touched.
- **Chips with no live Airtable choice color to read** (e.g. a linked-record field like a category, which has no per-choice color at all): use a hardcoded rainbow — but cycle through Airtable's own real choice-color hexes, not a synthetic evenly-spread HSL hue (`hue = 360 * index / totalOptionCount`, deprecated 2026-09-01 — Axel supplied Airtable's actual palette swatch grid as the reference). **Uses the Light1 tier** (the picker's second row of tones, per Axel, 2026-09-01 — was the Bright tier before that, one row down): `#9CC7FF` blue, `#71DCF5` cyan, `#63E6D3` teal, `#8AE28A` green, `#FFE07A` yellow, `#FFC582` orange, `#FF9AA6` red, `#FF9DEB` pink, `#C99BF5` purple, `#C6CBD1` gray — in that fixed order, `index % 10`. Order over the currently-active option list, sorted stably (e.g. alphabetically) so a given option's hue doesn't shift as unrelated records load. Same solid-background/dark-text (`#1D1F25`) rendering as above — this palette's Light1 tier still reads fine with dark text, no white-text exception needed. Implemented as `getRainbowHex`/`RAINBOW_PALETTE` (`refund_requests.tsx`), `getRefundRainbowHex`/`REFUND_RAINBOW_PALETTE` (`draft_orders.tsx`), `getSoldRainbowHex`/`SOLD_RAINBOW_PALETTE` (`sold_orders.tsx`) — same 10 hexes, copy-pasted per file (no shared module between Interface Extension files).
...
- **Resolving a choice color from a lookup field**: `field.options.choices` only works when `field` is the actual `singleSelect`/`multipleSelects` field. A `multipleLookupValues` field that mirrors one does **not** reliably expose the source field's option colors at runtime (confirmed empirically: it resolves 0 choices even though the same code path works fine for a direct single-select). Never assume a lookup exposes its source's colors — trace the lookup to its real source field (via the field's `recordLinkFieldId`/`fieldIdInLinkedTable` config, or just ask which table/field it mirrors) and read `.options.choices` from **that** field instead.
- **When wiring up any resolved-from-a-field color** (not just lookups — any time color/style is meant to track an Airtable field dynamically), add a temporary debug `title` tooltip on the element showing: the value, the field/table it's sourced from, the resolved color name, and how many choices loaded. Verify it in the live interface before treating the color as working, then it's safe to leave in or strip per the user's preference. This turns "the color looks wrong" into a one-screenshot diagnosis instead of a guessing loop.
